...
def extractMainLobe(window, M):
    ...
    ### Your code here
    x = np.zeros(8 * int(len(w)))
    N = len(x)
    x[:int(len(w))] = w
    fftx = fft(x)
    for i in range(N):
        if i > 0:
            if abs(fftx[i]) > abs(fftx[i-1]):
                # There are i-1 samples to the right of the 0-bin, so grab i-1 from the right side.
                # Apparently the instructor calculates the edge slightly differently, so not i-2.
                return(20 * np.log10(abs(np.append(fftx[N-(i-1):N], (fftx[0:i])))))
